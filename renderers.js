...
const TextRenderer = (() => {
  ...
  /* Колонка, в которой встал последний нарисованный набор: зона после маски
     и фактический центр блока ПОСЛЕ клампинга.

     Нужна переводу. Перевод рисуется отдельным вызовом draw, и до сих пор он
     получал ту же безопасную зону кадра и ту же исходную точку строки, что и
     основной набор, — а дальше считал всё сам: свою маску по СВОЕЙ высоте,
     свой перенос, свой клампинг центра. У основной строки и перевода высоты
     разные (перевод стоит ниже и кеглем мельче), поэтому маска давала им
     разные зоны: основная строка ужималась в узкую колонку рядом с фигурой,
     перевод — раскладывался по всей ширине кадра и центровался в другом
     месте. Две половины одного набора уезжали друг от друга — ровно то, на
     что жалуются: «текст сломался».

     Перевод — не самостоятельная строка, а вторая половина того же блока,
     поэтому колонку он обязан наследовать, а не вычислять заново. */
  ...
  function drawWordLayout(ctx, lyric, cx, cy, anim, fadeAlpha, color, font, fontSize, canvasWidth, t, globalBoxId, bounds, shapeAt, pass = null) {
    ...
    // Колонка word-layout — наружу, для перевода (см. _lastLayout).
    if (pass !== 'ghost') {
      _lastLayout = { area: { x: wlArea.x, y: wlArea.y, w: wlArea.w, h: wlArea.h }, cx };
    }
  ...
  function draw(ctx, lyric, cx, cy, anim, fadeAlpha, color, font, fontSize, canvasWidth, t=0, globalBoxId=null, bounds=null, shapeAt=null, pass=null) {
    ...
    /* Колонка этого набора — наружу, для перевода. Снимается ПОСЛЕ
       клампинга: переводу нужно место, где блок реально стоит. */
    if (pass !== 'ghost') _lastLayout = { area: { x: area.x, y: area.y, w: area.w, h: area.h }, cx };
  ...
  return { draw, getFadeAlpha, textShade: _deepShade,
           get lastLayout() { return _lastLayout; },
