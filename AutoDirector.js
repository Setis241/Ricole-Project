...
const AutoDirector = (function() {
  ...
  function buildScore(audio, lyr, sections, opts) {
    ...
    lyr.lines.forEach(function(l) {
      ...
      const stripped = raw
        /* POSX/POSY тут не было, и это било больнее всего: собственный тег
           прошлого прогона не снимался с исходной строки, оставался в тексте
           и на следующем прогоне становился ЧАСТЬЮ слов песни — а сверху
           дописывался новый. Каждое «применить партитуру» впекало в текст
           ещё один системный тег. */
        .replace(/\{L(?:FONT|SIZE|ANIM|COLOR|BGIMG|POSX|POSY|POS|LAYER|OVFX|NOBOX):[^}]+\}/g, '')
        .replace(/\{LNOBOX\}/g, '')
    ...
    /* ── РАМКА КЛИПА В САМОМ ТЕКСТЕ ──────────────────────────────
       Прощание пишется строкой-меткой [конец]: она РАБОЧАЯ — LRCParser
       поднимает по ней isEnding, и финал живёт от текста, а не от
       невидимого состояния движка. Строка без текста инструментальная,
       в кадр не попадает.

       ВСТУПЛЕНИЕ ЗДЕСЬ НЕ ПИШЕТСЯ, И ЭТО НЕ ЗАБЫВЧИВОСТЬ.

       Номер строки — это ключ, на котором держится половина проекта:
       по нему привязаны фоны (selectedLines в BackgroundManager),
       кам-сцены, хореография спрайта и построчные стили. Вставка строки
       В НАЧАЛО сдвигает все последующие номера на единицу, и вся эта
       привязка молча съезжает на соседнюю строку — фон не тот, сцена не
       та, спрайт стоит не там, где считали текст. Метка вступления
       ровно это и делала.

       Прощание такой беды не создаёт: его время позже последней строки,
       новая строка встаёт в конец, и номера 0..N-1 не двигаются. Ниже
       это ещё и проверяется явно.

       Где вступление видно вместо метки: отчёт панели режиссуры и
       window.RicoleFraming() в консоли. */
    const fr = opts.framing || null;

    /* Уборка за прошлой версией: пустая строка-метка вступления, если она
       уже попала в текст, номера строк держит сдвинутыми. Пока она там,
       фоны и спрайт остаются съехавшими, поэтому её надо убрать — иначе
       баг не лечится ничем, кроме ручной правки. Строки с текстом не
       трогаем никогда. */
    const outClean = out.filter(function(o) {
      if (o.text) return true;                       // строки с текстом — никогда
      // Метка стоит В НАЧАЛЕ, но за ней идут команды кадра, поэтому
      // проверяем именно ведущую метку, а не строку целиком.
      const lab = /^\s*\[([^\]]+)\]/.exec(o.rawText || '');
      return !(lab && INTRO_SECTION_RE.test(lab[1]));
    });
    out.length = 0;
    Array.prototype.push.apply(out, outClean);

    if (fr && fr.ending && fr.ending.source !== 'marker') {
      const hasEnd = lyr.lines.some(function(l) { return l.entry && l.entry.isEnding; });
      // Только если метка действительно встаёт ПОСЛЕ всех существующих
      // строк — иначе она сдвинет чьи-то номера (см. выше).
      const lastTime = lyr.lines.length ? lyr.lines[lyr.lines.length - 1].time : 0;
      if (!hasEnd && fr.ending.time >= lastTime) {
        out.push({ time: fr.ending.time, rawText: '[конец]', text: '' });
      }
    }
