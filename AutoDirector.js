...
const AutoDirector = (function() {
  ...
  /* Партитура анимаций.
     ...
     ВАЖНО — чередование идёт ВНУТРИ одного семейства движения:
       ...
       волна    ripple, liquid, pendulum_wave  бегущая волна по строке
  ...
  const ANIM = {
    ...
    // волна — бридж по смыслу «течёт», а не бьёт
    bridge:    { calm: ['pendulum_wave', 'parallax'],
                 normal: ['pendulum_wave', 'parallax'],
                 fast:   ['pendulum_wave', 'cascade'] },
    // текста нет — построчный режим, кадр живёт фоном
    break:     { calm: 'cinematic',             normal: 'cinematic',                fast: 'cinematic' },
    // спокойная развёртка — кольцует клип с интро
    outro:     { calm: ['parallax', 'cascade'], normal: ['cascade', 'parallax'],    fast: ['cascade', 'snap'] },
  };
  ...
  function _compose(type, idxInSec, isLast, anim) {
    ...
    if (type === 'verse' || type === 'prechorus' || type === 'outro') {
  ...
  const WORDFX = {
    ...
    chorus: 'GLOW', bridge: null, break: null, outro: null,
  };

  // Множитель размера от типа секции
  const SIZE = {
    intro: 0.95, verse: 0.90, prechorus: 1.02,
    chorus: 1.30, bridge: 0.85, break: 0.90, outro: 0.95,
  };
  ...
  function buildScore(audio, lyr, sections, opts) {
    ...
    lyr.lines.forEach(function(l) {
      ...
      const isVerseLike = (type === 'verse' || type === 'outro');
